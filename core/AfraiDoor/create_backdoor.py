...
print("Adding %.2f percent noise to input data sets"%(percent_noise*100) )


print('Poisoning input set')
with jsonlines.open(opt.src_jsonl_dir, 'r') as reader:
	with jsonlines.open(os.path.join(opt.dest_jsonl_dir, 'poisoned_output.jsonl'), 'w') as writer:
		c = 0
		clean = 0
		poisoned = 0
		skip = 0
